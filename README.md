...
Two **session stats pills** for the ZCode desktop app, pinned to
the whitespace below the chat composer, showing real-time token usage and performance
metrics of the current session. Click either pill to open a
detail dialog. Always
visible in chat views, independent of the chat layout.

为 ZCode 桌面版打造的**会话统计双胶囊**，固定在聊天输入框下方的留白区，
实时显示当前会话的 token 用量与性能指标，点击任一胶囊弹出详情弹层
（上下文占用不在此显示——ZCode 输入框自带上下文读数）。聊天视图内**始终显示**，
不依赖聊天区布局。
...
- **Two pills below the composer**, aligned with the DeepSeek
  Harness official design: a gauge pill (`N turns · M steps · X tok/s` +
  live dot) and a db pill (`total tok · cache hit %`). No context pill —
  the ZCode composer already shows context occupancy natively. Survives React
  re-renders, session switches, and layout rebuilds.
  **输入框下方双胶囊**，对齐 DeepSeek Harness 官方设计：仪表盘 pill（轮/步 + tok/s +
  状态点）与数据库 pill（累计 token · 缓存命中）。不做上下文胶囊——ZCode 输入框
  自带上下文读数，重复展示已移除。React 重渲染、切换会话、布局重建都不影响。
- **Click a pill for a detail dialog** — "Session stats" (LLM time, tool time, avg
  ...
  cache write / output, all exact token counts). Anchored above the trigger, closes
  on outside click / Escape, the dialogs are mutually exclusive, and content
...
| Context | Used = the last completed main-turn request's `input_tokens` (incl. cache read); window = the model's `contextWindow`. Not shown in the bar (the ZCode composer shows it natively); still emitted for `/stats` |
...
hooks(on-event.mjs)─write session pointer─┐
                                          ▼
ZCode DB ◄─read-only─┐
ZCode model config ◄─┴─ daemon.mjs ──HTTP 127.0.0.1:47771──► pills + dialogs
...
hooks(on-event.mjs)──写会话指针──┐
                                 ▼
ZCode 数据库 ◄──只读──┐
ZCode 模型配置 ◄──只读─┴─ daemon.mjs ──HTTP 127.0.0.1:47771──► 双胶囊 + 弹层
...
渲染端可以把视图/定位诊断 POST 给 daemon，写入 `daemon.log`（仅排障用）：
...
- ZCode 刚启动、守护进程尚未就绪时，仪表盘 pill 显示「会话统计 · 等待本地服务…」，
  就绪后自动切到真实指标。
- 不做上下文胶囊：ZCode 输入框自带上下文占用读数，悬浮条重复展示已移除
  （上下文数据仍在 daemon 聚合结果里，`/stats` 命令照常输出）。
- 排障：先在渲染器控制台打开诊断上报（`localStorage.setItem("zcstats:diag", "1")`），
  复现问题后查看 `~/.zcode/session-stats/daemon.log`。
